...
class AKDatePicker(BaseDialog, ThemableBehavior):
    ...
    def __init__(self, callback=None, **kwargs):
        ...
        for x in (range(self.year_range[0], self.year_range[1])):
            self.ids.year_view.add_widget(MDFlatButton(text=str(x), on_release=self._set_year))
        #for x in (range(1, 13)):
        for k, v in self.month_dic.items():
            ...
            self.ids.month_view.add_widget(MDFlatButton(text=month, on_release=self._set_month))
        for day in (self.day_list):
            self.ids.day_view.add_widget(MDFlatButton(text=day, on_release=self._set_day))
